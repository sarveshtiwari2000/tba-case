...
//Instance of the vehicle movement
public class vehicles extends JPanel{
    ...
    int nOpponent;      //the number of opponent vehicles in the vehicle movement
    String imageLoc[]; //array used to store opponent car images
    int lx[],ly[];  //integer arrays used to store the x and y values of the coming vehicles
    ...
    int speedOpponent[]; //integer array used to store the speed value of each opponent vehicle in the vehicle movement
    boolean isFinished; //boolean that will be used the end the vehicle movement when a collision occurs
    ...
    public vehicles(){
        ...
        car_x = car_y = 300;    //initial setting the user's car location to (300,300)
        ...
        speedOpponent = new int[20]; //integer array used to store the speed value of each opponent vehicle in the vehicle movement
        isFinished = false; //when false, vehicle movement is running, when true, vehicle movement has ended
        score = highScore = 0;  //initial setting the current score and the high score to zero
    }
    ...
    public void paint(Graphics g){
        ...
        try{
            ...
            if(this.nOpponent > 0){ //if there is more than one opponent car in the vehicle movement
        ...
        for(int i=0;i<nOpponent;i++){ //for all opponent cars
            ...
            if((ly[i] >= car_y && ly[i] <= car_y+46) || (ly[i]+46 >= car_y && ly[i]+46 <= car_y+46)){   //if the cars collide vertically
                if(car_x+87 >= lx[i] && !(car_x >= lx[i]+87)){  //and if the cars collide horizontally
                    ...
                    this.finish(); //end vehicle movement and print end message
                }
            }
        }
    }
    
    //function that will display message after user has lost the vehicle movement
    void finish(){
        isFinished = true;  //indicates that vehicle movement has finished to the rest of the program
        this.repaint();     //tells the window manager that the component has to be redrawn
        JOptionPane.showMessageDialog(this,"Vehicle Movement Over !!!", "Vehicle Movement", JOptionPane.YES_NO_OPTION);//displays a message saying vehicle movement over
        System.exit(ABORT); //terminate vehicle movement
    }
    ...
    public static void main(String args[]){
        JFrame frame = new JFrame("Vehicle Movement");   //creating a new JFrame window to display the vehicle movement
        vehicles movement = new vehicles(); //creating a new instance of a movement
        frame.add(movement);		//Graphics2D components are added to JFrame Window
        ...
        int count = 1, c = 1;
        while(true){
            movement.moveRoad(count);   //move the road
            while(c <= 1){
                movement.repaint();     //redraw road to match new locations
                try{
            ...
            if(movement.nOpponent < 4 && count % 200 == 0){ //if there is less than 4 cars and count timer reaches 200
                movement.imageLoc[movement.nOpponent] = "images/car_left_"+((int)((Math.random()*100)%3)+1)+".png"; //assign images to the opponent cars
                movement.lx[movement.nOpponent] = 499; //set opponent cars start positions
                ...
                movement.ly[movement.nOpponent] = p; //assign lane position to car
                movement.speedOpponent[movement.nOpponent] = (int)(Math.random()*100)%2 + 2; //sets the speed of the new opponent car to a random number that is the remainder of a number between 0 and 100, plus 2
                movement.nOpponent++; //add the car to the vehicle movement
            }
        }
    }
}
